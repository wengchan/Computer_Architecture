...
void next_line_prefetcher(struct cache_t *cp, md_addr_t addr) {
  /* compute the block-aligned address of the current access */
  md_addr_t curr_block_addr = CACHE_BADDR(cp, addr);

  /* compute next-line block address */
  md_addr_t next_block_addr = curr_block_addr + cp->bsize;

  /* Only prefetch if the next block is NOT already present in the cache */
  if (!cache_probe(cp, next_block_addr)) {

        /* Issue a prefetch access:
           - Read
           - next block address
           - NULL buffer (we don't need data)
           - cp->bsize bytes
           - time = 0 (timing is not simulated here)
           - user data pointer not needed → NULL
           - replaced block pointer not needed → NULL
           - prefetch = 1 to record stats correctly
        */
    cache_access(cp,Read,next_block_addr,NULL,cp->bsize,0,NULL,NULL,1);
  }
}

/* RPT state encoding: must match the 4 states in the handout */
enum rpt_state_t {
  RPT_STATE_INITIAL = 0,   /* init */
  RPT_STATE_TRANSIENT,     /* transient */
  RPT_STATE_STEADY,        /* steady */
  RPT_STATE_NOPRED         /* no-prediction */
};

struct rpt_entry_t {
  md_addr_t tag;           /* PC tag of the memory instruction */
  md_addr_t prev_addr;     /* last block address accessed by this PC */
  int       stride;        /* difference between last two block addresses */
  enum rpt_state_t state;  /* 2-bit FSM state */
  int confidence;
};

/* Global RPT (direct-mapped) */
static struct rpt_entry_t *rpt_table = NULL;
static int rpt_size = 32;     /* number of entries/


/* Open Ended Prefetcher */
void open_ended_prefetcher(struct cache_t *cp, md_addr_t addr) {
  extern md_addr_t get_PC();
  md_addr_t pc = get_PC();

  /* Tunable constants */
  const int CONF_MAX = 10;        /* saturating max */
  const int CONF_THRESHOLD = 3;   /* must reach this to prefetch */

  /* Lazy allocate RPT based on cp->prefetch_type */
  if (rpt_table == NULL) {
    rpt_table = (struct rpt_entry_t *)
      calloc(rpt_size, sizeof(struct rpt_entry_t));
  }
  ...
  md_addr_t curr_block = CACHE_BADDR(cp, addr);

  /* Direct-mapped RPT index:
     PCs in SimpleScalar are 8-byte aligned, so we shift by 3. */
  int index = (int)((pc >> 3) % rpt_size);
  struct rpt_entry_t *e = &rpt_table[index];

  /* ----------------------------------------------------
     CASE 1: Empty / different tag → initialize entry
     ---------------------------------------------------- */
  if (e->tag == 0 || e->tag != pc) {
    e->tag        = pc;
    e->prev_addr  = curr_block;
    e->stride     = 0;
    e->state      = RPT_STATE_INITIAL;
    e->confidence = 0;
    return;
  }

  /* ----------------------------------------------------
     CASE 2: Existing entry — update stride & FSM
     ---------------------------------------------------- */
  md_addr_t prev_block = e->prev_addr;
  int new_stride = (int)(curr_block - prev_block);

  /* Require stride to be block-aligned */
  if (new_stride % cp->bsize != 0)
    new_stride = 0;

  /* Optionally filter out huge random strides */
  if (abs(new_stride) > 4 * cp->bsize)
    new_stride = 0;

  int same_stride = (new_stride == e->stride);

  /* ------------------ FSM TRANSITIONS ------------------ */
  switch (e->state) {
  case RPT_STATE_INITIAL:
    if (same_stride) {
      e->state = RPT_STATE_STEADY;
    } else {
      e->state = RPT_STATE_TRANSIENT;
      e->stride = new_stride;
    }
    break;
  ...
  /* Update prev address for next round */
  e->prev_addr = curr_block;

  /* ----------------------------------------------------
     PREFETCH DECISION
     Must be (STEADY + confident + valid stride)
     ---------------------------------------------------- */
  if (e->state == RPT_STATE_STEADY &&
      e->stride != 0 &&
      e->confidence >= CONF_THRESHOLD) {

    md_addr_t pf_block = curr_block + e->stride;

    if (!cache_probe(cp, pf_block)) {
      cache_access(cp, Read, pf_block, NULL, cp->bsize, 0, NULL, NULL, 1);
    }
  }
}
...
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
	//using lab figure; 
  /* we need the PC of the load/store that caused this data access */
  extern md_addr_t get_PC();
  md_addr_t pc = get_PC();

  /* lazily allocate / initialize the RPT once */
  if (rpt_table == NULL) {
    rpt_table = (struct rpt_entry_t *)
      calloc(rpt_size, sizeof(struct rpt_entry_t));
  }
  ...
  /* Use block-aligned addresses for stride */
  md_addr_t curr_block_addr = CACHE_BADDR(cp, addr);

  /* direct-mapped index using PC */
  int index = (int)((pc >> 3) & (rpt_size - 1));
  struct rpt_entry_t *entry = &rpt_table[index];

  /* ---------- Scenario 1: no corresponding entry ---------- */
  if (entry->tag == 0 || entry->tag != pc) {
    entry->tag       = pc;
    entry->prev_addr = curr_block_addr;
    entry->stride    = 0;
    entry->state     = RPT_STATE_INITIAL;
  }
  else {
    /* ---------- Scenario 2: existing entry ---------- */
    md_addr_t prev_block_addr = entry->prev_addr;
    int new_stride = (int)(curr_block_addr - prev_block_addr);
    int same_stride = (new_stride == entry->stride);
    int update_stride = 0;
  ...
  /* ---------- Generate prefetch ---------- */
  if (entry->state == RPT_STATE_INITIAL ||
      entry->state == RPT_STATE_TRANSIENT ||
      entry->state == RPT_STATE_STEADY) {

    if (entry->stride != 0) {
      md_addr_t prefetch_block_addr = curr_block_addr + entry->stride;

      if (!cache_probe(cp, prefetch_block_addr)) {
        cache_access(cp,Read,prefetch_block_addr,NULL,
                     cp->bsize,0,NULL,NULL,1);
      }
    }
  }
}
